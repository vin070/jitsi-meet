...
class AbstractVideoManager extends Component<Props> {
    ...
    componentDidUpdate(prevProps: Props) {
        const { _status, _time, _videoUrl, _isOwner, _volume, _muted } = this.props;

        if (prevProps._videoUrl !== _videoUrl && _videoUrl) {
    ...
    /**
     * Handle changes to the shared paused video.
     *
     * @returns {void}
     */
    fireUpdatePausedVideoEvent() {
        if (this.getPlaybackState() === PLAYBACK_STATES.PAUSED) {
            this.fireUpdateSharedVideoEvent();
        }
    }
...
export function _mapStateToProps(state: Object): $Shape<Props> {
    const { ownerId, status, time, videoUrl, muted, volume } = state['features/shared-video'];
...
export function _mapDispatchToProps(dispatch: Function): $Shape<Props> {
    return {
        ...
        _setSharedVideoStatus: ({ videoUrl, status, time, ownerId, muted, volume }) => {
